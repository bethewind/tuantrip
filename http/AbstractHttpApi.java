...
import com.tudaidai.tuantrip.util.JSONUtils;
...
abstract public class AbstractHttpApi implements HttpApi {
	protected static final Logger LOG = Logger.getLogger(AbstractHttpApi.class
			.getCanonicalName());
	protected static final boolean DEBUG = TuanTripSettings.DEBUG;

	private static final String DEFAULT_CLIENT_VERSION = "com.tudaidai.tuantrip.tuan";
	...
	public TuanTripType executeHttpRequest(HttpRequestBase httpRequest,
			Parser<? extends TuanTripType> parser, int type, String charset)
			throws IOException, ParseException, TuanTripException,
			XmlPullParserException {
		...
		// while((sssString=bReader.readLine())!=null)
		// {
		// LOG.info(sssString);
		//
		// }
		...
		case 200:
			...
			default:
				throw new TuanTripException("there is no parser for type "
						+ type);
			}
		...
		case 400:
			...
			throw new TuanTripException(response.getStatusLine().toString(),
		...
		case 500:
			...
			throw new TuanTripException("Foursquare is down. Try again later.");
		...
		default:
			...
			response.getEntity().consumeContent();
			throw new TuanTripException("Error connecting to Foursquare: "
	...
	public TuanTripType executeHttpRequest(HttpRequestBase httpRequest,
		...
		return executeHttpRequest(httpRequest, parser, JSON, "utf-8");
	}

	public String doHttpPost(String url, NameValuePair... nameValuePairs)
			throws TuanTripParseException, TuanTripException, IOException {
		if (DEBUG)
		...
		case 200:
			try {
				...
				throw new TuanTripParseException(e.getMessage());
			}
